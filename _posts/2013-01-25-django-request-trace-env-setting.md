...
디버깅 쉘에서 많이 사용하는 커맨드는 다음과 같다.

- n : 가장많이 사용하는next입니다. 현재 라인을 실행시킵니다.
- s : 진행중인 라인에 함수가 포함되어 있으면 해당 함수 안으로 이동합니다.
- u : s 의 반대로 현재 진행중인 함수의 밖. 그러니가 상위스택으로 이동합니다.
- r : u와 비슷하지만 함수 밖으로 나가지는 않으며 현재 진행중인 함수의 return 부분까지 실행합니다.
- c : continue입니다. 다시 set_trace()를 만나기 전까지 코드를 실행합니다.
- q : 디버깅을 종료하고 프로그램을 종료시킵니다.
- b : 실행중에 임의로 break point를 만듭니다. b 10 과같이 입력하면 10번째 라인에 break point가 생성됩니다. 주의할것은 빈줄이나 코멘트 부분에는 적용되지 않는다는것 입니다.

이걸로 기본적인 트레이스 준비는 전부 끝났습니다.

이제 프로젝트를 생성하고 line by line으로 따라가시면됨.

불친절한 가이드 읽으시느라 수고 많으셨습니다.
문의사항은 코멘트로 남겨주시거나하면 수시로 적용할것으로 예상되어집니다.(=_=;)
